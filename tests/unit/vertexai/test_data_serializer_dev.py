...
class TestPandasDataSerializerDev:
    def test_base_case(self, tmp_path):
        fake_gcs_uri = "gs://staging-bucket/test_base_case.parquet"
        df = pd.DataFrame(np.zeros(shape=[3, 3]))
        df_serializer = serializers.PandasDataSerializerDev()

        df_serializer.serialize(df, fake_gcs_uri)
        restored_df = df_serializer.deserialize(tmp_path / "test_base_case.parquet")
        pd.testing.assert_frame_equal(df, restored_df)

    def test_base_case_with_nans(self, tmp_path):
        fake_gcs_uri = "gs://staging-bucket/test_base_case_with_nans.parquet"
        df = pd.DataFrame(np.zeros(shape=[3, 3]))
        df.iat[0, 0] = np.nan
        df_serializer = serializers.PandasDataSerializerDev()

        df_serializer.serialize(df, fake_gcs_uri)
        restored_df = df_serializer.deserialize(
            tmp_path / "test_base_case_with_nans.parquet"
        )
        pd.testing.assert_frame_equal(df, restored_df)

    def test_df_with_string_col_names_and_nans(self, tmp_path):
        fake_gcs_uri = (
            "gs://staging-bucket/test_df_with_string_col_names_and_nans.parquet"
        )
        df = pd.DataFrame(np.zeros(shape=[3, 3]))
        df.columns = ["0", "1", "2"]
        df.iat[0, 0] = np.nan
        ...
        df_serializer.serialize(df, fake_gcs_uri)
        restored_df = df_serializer.deserialize(
            tmp_path / "test_df_with_string_col_names_and_nans.parquet"
        )
        pd.testing.assert_frame_equal(df, restored_df)

    def test_df_with_mixed_data_types_and_nans(self, tmp_path):
        fake_gcs_uri = (
            "gs://staging-bucket/test_df_with_mixed_data_types_and_nans.parquet"
        )
        df = pd.DataFrame(np.zeros(shape=[3, 3]))
        df.columns = ["0", "1", "2"]
        ...
        df_serializer.serialize(df, fake_gcs_uri)
        restored_df = df_serializer.deserialize(
            tmp_path / "test_df_with_mixed_data_types_and_nans.parquet"
        )
        pd.testing.assert_frame_equal(df, restored_df)
    ...
    def test_df_with_mixed_data_types_and_categorical_data(self, tmp_path):
        fake_gcs_uri = (
            "gs://staging-bucket/test_df_with_mixed_data_types_and_cat_data.parquet"
        )
        df = pd.DataFrame(np.zeros(shape=[3, 3]))
        df.columns = ["0", "1", "2"]
        df.insert(3, "string_col", ["0", np.nan, "2"])
        ...
        df_serializer.serialize(df, fake_gcs_uri)
        restored_df = df_serializer.deserialize(
            tmp_path / "test_df_with_mixed_data_types_and_cat_data.parquet"
        )
        pd.testing.assert_frame_equal(df, restored_df)
    ...
    def test_df_with_multiindex_and_all_string_indices(self, tmp_path):
        fake_gcs_uri = (
            "gs://staging-bucket/test_df_with_multiindex_and_all_string_indices.parquet"
        )
        arrays = [
        ...
        df_serializer.serialize(df, fake_gcs_uri)
        restored_df = df_serializer.deserialize(
            tmp_path / "test_df_with_multiindex_and_all_string_indices.parquet"
        )
        pd.testing.assert_frame_equal(df, restored_df)
